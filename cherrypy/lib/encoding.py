    ...
    def find_acceptable_charset(self):
        ...
        if self.encoding is not None:
            ...
            if not encs:
                if self.debug:
                    cherrypy.log(
                        'Attempting default encoding %r' % self.default_encoding,
                        'TOOLS.ENCODE',
                    )
                ...
                for element in encs:
                    if element.qvalue > 0:
                        if element.value == '*':
                            ...
                            if self.debug:
                                cherrypy.log(
                                    'Attempting encoding %s (qvalue >' '0)' % element,
                                    'TOOLS.ENCODE',
                                )
                            if encoder(encoding):
                                return encoding
                ...
                if '*' not in charsets:
                    ...
                    if iso not in charsets:
                        if self.debug:
                            cherrypy.log(
                                'Attempting ISO-8859-1 encoding', 'TOOLS.ENCODE'
                            )
                        if encoder(iso):
                            return iso
        ...
        if ct and self.add_charset:
            ...
            if self.text_only:
                if ct.value.lower().startswith('text/'):
                    if self.debug:
                        cherrypy.log(
                            'Content-Type %s starts with "text/"' % ct, 'TOOLS.ENCODE'
                        )
                    do_find = True
                else:
                    if self.debug:
...
def gzip(compress_level=5, mime_types=['text/html', 'text/plain'], debug=False):
    ...
    for coding in acceptable:
        if coding.value == 'identity' and coding.qvalue != 0:
            if debug:
                cherrypy.log(
                    'Non-zero identity qvalue: %s' % coding, context='TOOLS.GZIP'
                )
            return
        if coding.value in ('gzip', 'x-gzip'):
            ...
            if ct not in mime_types:
                ...
                if not found:
                    if debug:
                        cherrypy.log(
                            'Content-Type %s not in mime_types %r' % (ct, mime_types),
                            context='TOOLS.GZIP',
                        )
                    return

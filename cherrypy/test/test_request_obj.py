...
class RequestObjectTests(helper.CPWebCase):
    ...
    def setup_server():
        ...
        appconf = {
            '/method': {
                'request.methods_with_bodies': ('POST', 'PUT', 'PROPFIND', 'PATCH')
            },
        }
    ...
    def testParams(self):
        ...
        self.assertBody(
            'args: %s kwargs: %s'
            % (('\xd4 \xe3', 'cheese'), [('Gruy\xe8re', ntou('Bulgn\xe9ville'))])
        )
        ...
        self.assertBody(
            'args: %s kwargs: %s'
            % (('code',), [('url', ntou('http://cherrypy.dev/index?a=1&b=2'))])
        )
    ...
    def testErrorHandling(self):
        ...
        try:
            ...
            if cherrypy.server.protocol_version == 'HTTP/1.0' or getattr(
                ...
                self.assertRaises(
                    (ValueError, IncompleteRead), self.getPage, '/error/page_streamed'
                )
    ...
    def test_encoded_headers(self):
        ...
        if cherrypy.server.protocol_version == 'HTTP/1.1':
            ...
            self.getPage(
                '/headers/ifmatch', [('If-Match', ntou('=?utf-8?q?%s?=') % (c * 10))]
            )
    ...
    def test_basic_HTTPMethods(self):
        ...
        self.getPage(
            '/method/parameterized', method='PATCH', body='data=on+top+of+other+things'
        )
        ...
        self.getPage(
            '/method/parameterized', method='PUT', body='data=on+top+of+other+things'
        )
        self.assertBody('on top of other things')
